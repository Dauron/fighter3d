...
void main()
{
    ...
    vec3  aux  = gl_LightSource[0].position.xyz-position;
    ...
    if (NdotL < 0.001)
    {
        gl_FragData[0] = vec4(attenuation * (material.rgb * gl_LightSource[0].ambient.rgb), material.a);
    ...
    gl_FragData[0] = vec4(attenuation * material.rgb *
                          ( gl_LightSource[0].ambient.rgb
                          + NdotL * gl_LightSource[0].diffuse.rgb ), material.a);
    gl_FragData[1] = vec4(attenuation * specular * gl_LightSource[0].specular.rgb, 0.0);
}