...
class PlayState : public BaseState {
            ...
            if (g_InputMgr.GetInputStateAndClear(IC_LClick))
            {
                ...
                if (PlayButton.Contains(x,y))
                {
                    ...
                    *choosen[1] = SkeletizedObj();
                    g_Application.SetCurrentScene(new SceneGame(player1,player2));
                        ...
                        xDWORD Width, xDWORD Height)
    {
        ...
        pFont05->PrintF((Width - textLen) * 0.5f, Height-lineHeight05*1.25f, 0.0f, title);
        ...
        if (MenuButton.Contains(g_InputMgr.mouseX, g_InputMgr.mouseY))
            ...
            glColor4f( 1.0f, 1.0f, 1.0f, 1.f );
        pFont05->PrintF(MenuButton.X, lineHeight05*0.75f, 0.0f, menu);
        ...
        if (PlayButton.Contains(g_InputMgr.mouseX, g_InputMgr.mouseY))
            ...
            glColor4f( 1.0f, 1.0f, 1.0f, 1.f );
        pFont05->PrintF(PlayButton.X, lineHeight05*0.75f, 0.0f, play);
        ...
        for(int i = 0; i < 2; ++i)
        {
            ...
            pFont03->PrintF((WidthHalf - textLen) * 0.5f, y, 0.0f, controlType);
            y -= lineHeight03*2;

            ////// Fighters
            textLen = pFont03->Length("Fighters:");
            pFont03->PrintF(WidthHalf * 0.1f, y, 0.0f, "Fighters:");
            fightersY = Height - (xDWORD)y;
            y -= lineHeight03;
            for (int j = 0; j < players[i].size(); ++j)
            {
            ...
            stylesY = Height - (xDWORD)y;
            y -= lineHeight03;
            for (int j = 0; j < choosen[i]->styles.size(); ++j)
            {
